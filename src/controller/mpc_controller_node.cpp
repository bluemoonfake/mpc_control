#include "mpc_controller/controller/translational_mpc.hpp"
#include "mpc_controller/controller/waypoint_mission.hpp"
  ...
  std::array<double, 3> update(const std::array<double, 3> &measured,
    ...
    for (std::size_t axis = 0; axis < 2; ++axis) {
      ...
      const double predicted =
          alpha * value[axis] + (1.0 - alpha) * command[axis];
...
public:
  MpcControllerNode() : Node("mpc_controller_node") {
    ...
    declareAndGet("waypoint_acceptance_speed_m_s",
                  waypoint_acceptance_speed_m_s_);
    declareAndGet("strict_validation", strict_validation_);
    declareAndGet("cold_start_on_waypoint_transition",
                  cold_start_on_waypoint_transition_);
    ...
    declareAndGet("solver_absolute_tolerance",
                  config_.solver_absolute_tolerance);
    declareAndGet("solver_relative_tolerance",
                  config_.solver_relative_tolerance);
    ...
    declareAndGet("min_collective_specific_force_m_s2",
                  config_.min_collective_specific_force_m_s2);
    ...
    const bool backend_valid =
        parseBackend(controller_backend_, config_.backend);
    ...
    declare_parameter("model_time_constant_xyz",
    ...
    const bool vector_parameters_valid =
        ...
        getArrayParameter("model_time_constant_xyz",
                          config_.model_time_constant_xyz);
    const bool update_rate_valid =
        std::isfinite(update_rate_hz_) && update_rate_hz_ > 0.0;
    config_valid_ =
        vector_parameters_valid && backend_valid && update_rate_valid &&
        ...
        std::isfinite(waypoint_acceptance_speed_m_s_) &&
        waypoint_acceptance_speed_m_s_ > 0.0 && !output_frame_id_.empty() &&
    ...
    mapping_config_valid_ =
        mpc_controller::force_attitude::validParameters(mapping_config_);
    ...
    mpc_plan_subscription_ = create_subscription<MpcPlan>(
        "mpc_mission_plan", rclcpp::QoS(1).reliable().transient_local(),
        [this](MpcPlan::SharedPtr message) {
          mpcPlanCallback(std::move(message));
        });
    output_publisher_ =
        create_publisher<Output>("mpc_translational_output", 10);
    setpoint_publisher_ =
        create_publisher<Setpoint>("force_attitude_setpoint", 10);
    mission_completed_publisher_ = create_publisher<std_msgs::msg::Bool>(
        "/reference_generator_node/mission_completed", 10);

    if (!config_valid_) {
      RCLCPP_ERROR(get_logger(),
                   "Invalid MPC configuration; controller updates disabled");
      return;
    }
    if (!mapping_config_valid_) {
      RCLCPP_ERROR(
          get_logger(),
          "Invalid force/attitude control configuration; output disabled");
    }

    controller_.emplace(config_);
    RCLCPP_INFO(get_logger(), "MPC backend selected at startup: %s",
                controller_backend_.c_str());
    RCLCPP_INFO(
        get_logger(), "Waypoint-transition solver cold start: %s",
        cold_start_on_waypoint_transition_ ? "enabled" : "disabled");
...
private:
  ...
  using WaypointGoal = mpc_controller::translational::WaypointGoal;
  using SolveLimits = mpc_controller::translational::SolveLimits;
  using FailureReason = mpc_controller::translational::FailureReason;
  using WaypointMission = mpc_controller::controller::WaypointMission;

  static bool
  parseBackend(const std::string & /*name*/,
               mpc_controller::translational::Backend &backend) noexcept {
  ...
  bool getArrayParameter(const std::string &name,
                         std::array<double, 3> &output) {
    const auto values = get_parameter(name).as_double_array();
    if (values.size() != output.size()) {
      RCLCPP_ERROR(get_logger(), "Parameter '%s' must contain exactly 3 values",
                   name.c_str());
      config_valid_ = false;
      return false;
    }
  ...
  static bool convertReference(const Reference &message,
                               ReferenceData &output) noexcept {
  ...
  void referenceCallback(Reference::SharedPtr message) {
    ...
    if ((message->header.stamp.sec == 0 &&
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "MPC reference rejected: malformed, empty, "
                           "zero-timestamp or non-finite trajectory");
      return;
    }
  ...
  static std::vector<WaypointMission::Target>
  missionTargets(const MpcPlan &plan) {
    std::vector<WaypointMission::Target> targets;
    targets.reserve(plan.waypoints.size());
    for (const auto &source : plan.waypoints) {
      WaypointMission::Target target;
      target.id = source.id;
      target.position = source.position;
      target.acceptance_radius_m = source.acceptance_radius_m;
      target.hold_duration_s = source.hold_duration_s;
      target.horizontal_speed_m_s = source.horizontal_speed_m_s;
      target.vertical_speed_m_s = source.vertical_speed_m_s;
  ...
  void installMission(const std::vector<WaypointMission::Target> &targets) {
    if (!state_) {
      pending_mission_targets_ = targets;
      return;
    }
    WaypointMission::VehicleState initial_state;
    initial_state.position = state_->position;
    initial_state.velocity = state_->velocity;
    initial_state.yaw = state_->yaw;
    WaypointMission mission(waypoint_acceptance_speed_m_s_);
    if (!mission.load(targets, initial_state)) {
      RCLCPP_ERROR(get_logger(), "MPC mission plan rejected: invalid waypoint data");
      mission_.reset();
      pending_mission_targets_.reset();
      return;
    }
    controller_->reset();
    ...
    pending_mission_targets_.reset();
    RCLCPP_INFO(get_logger(), "MPC mission %lu loaded with %zu waypoints",
                static_cast<unsigned long>(direct_mission_id_), targets.size());
  }

  void mpcPlanCallback(MpcPlan::SharedPtr message) {
    if (!message) {
      return;
    }
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    const auto targets = missionTargets(*message);
    if (targets.empty()) {
      RCLCPP_ERROR(get_logger(), "MPC mission plan rejected: no waypoints");
      return;
    }
    direct_mission_id_ = message->mission_id;
    installMission(targets);
  }

  SolveLimits
  missionLimits(const WaypointMission::Target &target) const noexcept {
    const auto bounded = [](double requested, double hard_limit) {
      return std::isfinite(requested) && requested > 0.0 ? std::min(requested, hard_limit) : hard_limit;
    };
    return {
        bounded(target.horizontal_speed_m_s, config_.max_speed_xy),
        bounded(target.vertical_speed_m_s, config_.max_speed_z),
        bounded(target.maximum_acceleration_m_s2, config_.max_acceleration_xy),
        bounded(target.maximum_acceleration_m_s2, config_.max_acceleration_z),
        bounded(target.maximum_jerk_m_s3, config_.max_control_rate_xy),
        bounded(target.maximum_jerk_m_s3, config_.max_control_rate_z)};
  }
  ...
  void update() {
    ...
    const double mutex_wait_seconds = std::chrono::duration<double>(
      mutex_acquired - callback_start).count();
    const double callback_period_seconds =
        callback_timing_initialized_
            ? std::chrono::duration<double>(callback_start - last_callback_start_)
                  .count()
            : 0.0;
    ...
    if (pending_mission_targets_) {
      installMission(*pending_mission_targets_);
    }
    const bool direct_mission_active = mission_.has_value();
    if (!direct_mission_active && !reference_) {
      return;
    }
    const auto now = get_clock()->now();
    const double reference_age = direct_mission_active ? 0.0 : (now - reference_received_at_).seconds();
    const double state_age = (now - state_received_at_).seconds();
    const bool stale_input = !std::isfinite(reference_age) || reference_age < 0.0 ||
                             (!direct_mission_active && reference_age > reference_timeout_seconds_) ||
    ...
    ReferencePoint command_reference;
    std::optional<WaypointMission::Update> mission_update;
    std::optional<SolveLimits> direct_limits;
    mpc_controller::translational::UpdateResult result;
    if (direct_mission_active) {

      WaypointMission::VehicleState mission_state;
      mission_state.position = measured.position;
      mission_state.velocity = measured.velocity;
      mission_state.yaw = state_->yaw;
      mission_update = mission_->update(mission_state, now.seconds());

      if (!mission_update->active) {
        return;
      }
      command_reference.position = mission_update->goal.target.position;
      command_reference.yaw = mission_update->goal.yaw;
      command_reference.yaw_rate = mission_update->goal.yaw_rate_rad_s;
      direct_limits = missionLimits(mission_update->goal.target);
      const bool waypoint_transition = mission_update->target_accepted &&
                                       !mission_update->completed_now;
      if (cold_start_on_waypoint_transition_ && waypoint_transition) {
        controller_->resetWarmStart();
        RCLCPP_INFO(get_logger(),
                    "MPC waypoint transition: solver warm start reset");
      }
      result = controller_->update(measured, WaypointGoal{command_reference.position}, *direct_limits);

      if (mission_update->target_accepted) {
        const auto &reached = mission_update->reached_target;
        const double dx = measured.position[0] - reached.position[0];
        const double dy = measured.position[1] - reached.position[1];
        const double dz = measured.position[2] - reached.position[2];
        const double speed = std::sqrt(measured.velocity[0] * measured.velocity[0] +
                                       measured.velocity[1] * measured.velocity[1] +
                                       measured.velocity[2] * measured.velocity[2]);
        RCLCPP_INFO(get_logger(),
                    "MPC waypoint %zu/%zu accepted: id='%s' distance=%.2f m speed=%.2f m/s",
                    mission_update->reached_index + 1U, mission_update->goal.count, reached.id.c_str(),
                    std::sqrt(dx * dx + dy * dy + dz * dz), speed);
      }
      if (mission_update->completed_now) {
        std_msgs::msg::Bool completed;
        completed.data = true;
        mission_completed_publisher_->publish(completed);
        RCLCPP_INFO(get_logger(),"MPC mission completed");
      }
    } else {
      ...
      command_reference = horizon.points.front();
      result = controller_->update(measured, horizon);
    }
    Output output;
    output.header.stamp = now;
    output.header.frame_id = output_frame_id_;
    output.trajectory_id = direct_mission_active ? direct_mission_id_ : reference_trajectory_id_;
    ...
    output.direct_waypoint_mode = direct_mission_active;
    if (direct_mission_active) {
      output.mission_id = direct_mission_id_;
      output.active_waypoint_index = static_cast<uint32_t>(mission_update->goal.index);
      output.active_waypoint_count = static_cast<uint32_t>(mission_update->goal.count);
      output.active_target_position = command_reference.position;
      output.active_max_speed_xy = direct_limits->max_speed_xy;
      output.active_max_speed_z = direct_limits->max_speed_z;
      output.active_max_acceleration_xy = direct_limits->max_acceleration_xy;
      output.active_max_acceleration_z = direct_limits->max_acceleration_z;
      output.active_max_control_rate_xy = direct_limits->max_control_rate_xy;
      output.active_max_control_rate_z = direct_limits->max_control_rate_z;
    }
    ...
    output.osqp_data_update_seconds = result.osqp_data_update_seconds;
    output.osqp_settings_update_seconds = result.osqp_settings_update_seconds;
    output.warm_start_seconds = result.warm_start_seconds;
    output.warm_start_prepare_seconds = result.warm_start_prepare_seconds;
    output.osqp_warm_start_seconds = result.osqp_warm_start_seconds;
    output.osqp_solve_seconds = result.osqp_solve_seconds;
    output.result_postprocess_seconds = result.result_postprocess_seconds;
    output.remaining_budget_before_osqp_seconds = result.remaining_budget_before_osqp_seconds;
    ...
    output.osqp_solve_cpu_seconds = result.osqp_solve_cpu_seconds;
    ...
    if (!result.valid) {
      const auto recovery = recoveryAcceleration(measured, command_reference);
      accel_observer_.setCommand(recovery);
      output.control_input = recovery;
      output.recovery_command_active = true;
      output.valid = false;
      recordCallbackTiming(output, callback_start, callback_period_seconds,
                           mutex_wait_seconds);
      output_publisher_->publish(output);
      publishSetpoint(output.header, output.sequence, recovery, state_->yaw, 0.0, true);
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
    ...
    RCLCPP_INFO_THROTTLE(
        ...
        raw_acceleration[1], raw_acceleration[2], result.control[0],
        result.control[1], result.control[2], reference_age * 1.0e3,
    ...
    publishSetpoint(output.header, output.sequence, result.control,command_reference.yaw, command_reference.yaw_rate, false);
  }
  ...
  void recordCallbackTiming(
    ...
    output.callback_period_jitter_seconds =
        callback_period_seconds > 0.0
            ? callback_period_seconds - 1.0 / update_rate_hz_
            : 0.0;
    output.callback_execution_seconds =
  ...
  std::array<double, 3>
  recoveryAcceleration(const MeasuredState &measured, const ReferencePoint &reference) const noexcept {
    ...
    std::array<double, 3> command{
        ...
        recovery_position_gain_z_ * (reference.position[2] - measured.position[2]) - recovery_velocity_gain_ * measured.velocity[2]};
    ...
    command[2] = std::clamp(command[2], -recovery_max_acceleration_z_, recovery_max_acceleration_z_);
    return command;
  }
  ...
  void updateShadowAdmission(
    ...
    const bool stable =
        result.coupled.valid && !result.deadline_missed &&
        ...
        result.coupled.max_constraint_violation <= 10.0 * (config_.solver_absolute_tolerance + config_.solver_relative_tolerance) &&
        result.coupled.max_predicted_tilt_rad <= config_.max_tilt_rad + 1.0e-6 &&
        result.coupled.max_predicted_collective_specific_force_m_s2 <= config_.max_collective_specific_force_m_s2 + 1.0e-6;
    if (!stable && shadow_admission_samples_ > 0U) {
      RCLCPP_WARN_THROTTLE(
          ...
          static_cast<unsigned>(result.coupled.status),
          result.coupled.iterations, result.coupled_solve_time_seconds * 1.0e3,
    ...
    shadow_admission_samples_ = stable ? std::min(shadow_admission_samples_ + 1U, shadow_admission_required_samples_) : 0U;
  ...
  void publishSetpoint(const std_msgs::msg::Header &header, uint64_t sequence,
    ...
    if (!mapping.valid) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           ...
                           mpc_controller::force_attitude::failureReasonName(
    ...
    for (int i = 0; i < 3; ++i) {
      setpoint.desired_acceleration_m_s2[i] =
          mapping.desired_acceleration_m_s2[i];
    ...
    if (!std::isfinite(yaw_rate)) {
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 2000,
          "Force/attitude output rejected: non-finite yaw rate");
      return;
    }
  ...
  std::optional<WaypointMission> mission_;
  std::optional<std::vector<WaypointMission::Target>> pending_mission_targets_;
  ...
  rclcpp::Subscription<MpcPlan>::SharedPtr mpc_plan_subscription_;

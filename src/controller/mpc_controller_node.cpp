...
public:
  MpcControllerNode() : Node("mpc_controller_node") {
    ...
    reference_subscription_ = create_subscription<Reference>("reference_trajectory", 10, [this](Reference::SharedPtr message) {
          referenceCallback(std::move(message));
        });
    state_subscription_ = create_subscription<State>("vehicle_state", 10, [this](State::SharedPtr message) {
    ...
    RCLCPP_INFO(get_logger(), "MPC backend selected at startup: %s", controller_backend_.c_str());
...
private:
  ...
  static bool
  parseBackend(const std::string & /*name*/, mpc_controller::translational::Backend &backend) noexcept {
  ...
  void referenceCallback(Reference::SharedPtr message) {
    ...
    if ((message->header.stamp.sec == 0 &&
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "MPC reference rejected: malformed, empty, zero-timestamp or non-finite trajectory");
      return;
    }
  ...
  void update() {
    ...
    if (stale_input) {
      if (!stale_input_active_) {
        ...
        // by the first-input rate constraint.  Restart from neutral
        // acceleration when fresh measurements return instead of reusing the
        // pre-gap state.
        controller_->reset();
    ...
    if (stale_input_active_) {
      RCLCPP_INFO(get_logger(),
              "Input-rate memory were reset: reference_age=%.3f s, state_age=%.3f s",
    ...
    if (strict_validation_) {
      if (!accel_gate_.accept(raw_acceleration)) {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                             "MPC update rejected: acceleration is non-finite "
                             "or |az|=%.3f exceeds %.3f m/s^2",
                             std::abs(raw_acceleration[2]), accel_gate_.max_z);
        return;
      }
    }
    measured.acceleration = accel_observer_.update(
        raw_acceleration, config_.model_time_constant_xyz, state_stamp_,
        state_timeout_seconds_);

    const double elapsed = now.seconds() - reference_->header_time_seconds;
    Horizon horizon;
    if (!Sampler::buildHorizon(*reference_, elapsed, config_.dt_first,
                               config_.dt_later, horizon)) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                           "MPC update rejected: reference cannot be sampled "
                           "on the solver grid");
      return;
    }
    ...
    for (std::size_t axis = 0; axis < 3; ++axis) {
      ...
      output.recovery_constraint_active[axis] =
          result.coupled.recovery_constraint_active;
    }
    ...
    output.coupled_max_constraint_violation =
        result.coupled.max_constraint_violation;
    output.coupled_max_speed_xy = result.coupled.max_predicted_speed_xy;
    output.coupled_max_acceleration_xy =
        result.coupled.max_predicted_acceleration_xy;
    ...
    output.shadow_control_difference_norm =
        result.shadow_control_difference_norm;
    ...
    solve_time_max_seconds_ =
        std::max(solve_time_max_seconds_, result.solve_time_seconds);
    if (!result.valid) {
      const auto recovery =
          recoveryAcceleration(measured, horizon.points.front());
      ...
      publishSetpoint(output.header, output.sequence, recovery, state_->yaw,
                      0.0, true);
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 2000,
    ...
    RCLCPP_INFO_THROTTLE(
        ...
        (solve_time_total_seconds_ / static_cast<double>(solve_count_)) *
            1000.0,
        solve_time_max_seconds_ * 1000.0,
        result.coupled.valid ? "true" : "false",
        result.coupled.max_predicted_tilt_rad,
        result.coupled.max_predicted_collective_specific_force_m_s2);
    publishSetpoint(output.header, output.sequence, result.control,
                    command_reference.yaw, command_reference.yaw_rate, false);
  }

  std::array<double, 3>
  recoveryAcceleration(const MeasuredState &measured,
                       const ReferencePoint &reference) const noexcept {
    ...
    std::array<double, 3> command{
        ...
        recovery_position_gain_z_ *
                (reference.position[2] - measured.position[2]) -
    ...
    command[2] = std::clamp(command[2], -recovery_max_acceleration_z_,
  ...
  void updateShadowAdmission(
    ...
    const bool stable =
        ...
        result.coupled.max_constraint_violation <=
            10.0 * (config_.solver_absolute_tolerance +
                    config_.solver_relative_tolerance) &&
        result.coupled.max_predicted_tilt_rad <=
            config_.max_tilt_rad + 1.0e-6 &&
        result.coupled.max_predicted_collective_specific_force_m_s2 <=
            config_.max_collective_specific_force_m_s2 + 1.0e-6;
    if (!stable && shadow_admission_samples_ > 0U) {
      RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 2000,
          "Coupled shadow admission reset: valid=%s deadline=%s status=%u "
          "iter=%d "
    ...
    shadow_admission_samples_ =
        stable ? std::min(shadow_admission_samples_ + 1U,
                          shadow_admission_required_samples_)
               : 0U;
    shadow_admission_ready_ =
        shadow_admission_samples_ >= shadow_admission_required_samples_;
  }
  ...
  void publishSetpoint(const std_msgs::msg::Header &header, uint64_t sequence,
    ...
    mapping_input.desired_acceleration_m_s2 =
        Eigen::Vector3d(acceleration[0], acceleration[1], acceleration[2]);
    ...
    const auto mapping =
        mpc_controller::force_attitude::compute(mapping_config_, mapping_input);
    if (!mapping.valid) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,

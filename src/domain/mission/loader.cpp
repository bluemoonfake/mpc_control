...
bool readFinite(
  const nlohmann::json & object, const char * name, double & output,
  bool allow_zero, std::string & error)
{
  ...
  error = std::string(name) + " must be finite and " + (allow_zero ? "non-negative" : "positive");
  return false;
}

bool readTpmc(const nlohmann::json & object, double & acceleration, double & jerk, std::string & error)
{
...
Mission MissionJsonParser::load(const std::string & source) const
{
  ...
  try {
      ...
      mission_json["defaults"].is_object())
    {
      ...
      if (!readFinite(
          defaults_json, "horizontalVelocity",
          parsed_mission.defaults.horizontal_velocity_m_s, false,
          parsed_mission.error) ||
        !readFinite(
          defaults_json, "verticalVelocity",
          parsed_mission.defaults.vertical_velocity_m_s, false,
          parsed_mission.error) ||
        !readFinite(
          defaults_json, "maxHeadingRate",
          parsed_mission.defaults.max_heading_rate_deg_s, false,
          parsed_mission.error) ||
        !readTpmc(
          defaults_json, parsed_mission.defaults.maximum_acceleration_m_s2,
          parsed_mission.defaults.maximum_jerk_m_s3, parsed_mission.error))
      {
        return parsed_mission;
      }
    }
    ...
    for (const auto & item_json : mission_json["items"]) {
      ...
      if (type == "takeoff") {
        ...
        if (!readFinite(
            item_json, "horizontalVelocity",
            item.settings.horizontal_velocity_m_s, false, parsed_mission.error) ||
          !readFinite(
            item_json, "verticalVelocity",
            item.settings.vertical_velocity_m_s, false, parsed_mission.error) ||
          !readFinite(
            item_json, "maxHeadingRate",
            item.settings.max_heading_rate_deg_s, false, parsed_mission.error) ||
          !readTpmc(
            item_json, item.settings.maximum_acceleration_m_s2,
...
RuntimeMissionLoader::Result RuntimeMissionLoader::loadAndStart(

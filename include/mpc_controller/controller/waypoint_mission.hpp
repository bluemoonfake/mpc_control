...
namespace mpc_controller::controller {

class WaypointMission final {
public:
  struct Target {
    std::string id;
  ...
  explicit WaypointMission(double acceptance_speed_m_s)
...
private:
  ...
  bool accepted(const VehicleState &state,
  ...
  Goal currentGoal(double now_seconds) {
    ...
    const double desired_yaw = std::isfinite(target.heading_rad)
                                   ? target.heading_rad
                                   : courseYaw(target.position);
    const double dt = std::isfinite(last_update_at_s_)
                          ? std::max(0.0, now_seconds - last_update_at_s_)
                          : 0.0;
  ...
  double
  courseYaw(const std::array<double, 3> &target_position) const noexcept {
